...
const CharacterPopularityGauge = (props) => {
  ...
  useEffect(() => {
    ...
    const width =
      parseInt(d3.select("#popularityGauge").style("width")) -
      margin.left -
      margin.right;
    const height =
    ...
    console.log("tickColour", tickColour);
    const big_ticks = () => {
    ...
    const numbers = () => {
      ...
      range.forEach((x) => {
        ...
        const radians = (x * 0.1375 - 0.6) * Math.PI;
    ...
    const moveNeedleTo =
      props.selectedCharacter == null
        ? 0
        : characterPopularityData[props.selectedCharacter] / 100;
  ...
  return (
    <span id="popularityGauge">
      <svg ref={d3Chart}></svg>
    </span>
  );
};

export default CharacterPopularityGauge;
